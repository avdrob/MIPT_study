 ...
 *		СОВМЕЩЕНИЯ ВЫЧИСЛЬНЫХ ОПЕРАЦИЯ С ДОСТУПОМ К ПАМЯТИ
 *		===================================================
 *
...
// КОНФИГУРАЦИИЯ
#define BLOCK_SIZE		(4*M)			// размер блока памяти
...
main()
{
	...
	// TITLE
	PRINT("= = = демонстрация совмещения вычислений с доступом к памяти = = =\n");
	PRINT_TITLE;

	/* -----------------------------------------------------------------------
	 *
	 *			cовмещение вычислений x+=cos(x) с чтением памяти
	 *
	------------------------------------------------------------------------ */
	printf("### x+=cos(x) ###\n");

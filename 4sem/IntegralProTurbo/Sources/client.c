...
const char *order="Wrong argumants\nb should be greater than a\n";
...
int *distr_task(double a, double b, long int total_segnum, int calcnum, calcmsg_t *calcs)
{
    ...
    for(i=1; i<calcnum-1; i++){
        ...
        tasks[i].lend=tasks[i-1].rend;
        tasks[i].rend=tasks[i].lend+cur_seg_prop_num*segm;
        tasks[i].thr_num=calcs[i].streamnum;
        tasks[i].seg_num=cur_seg_prop_num;
        //tasks[i]=(task_t){.rend=tasks[i].lend+cur_seg_prop_num*segm, .thr_num=calcs[i].streamnum, .seg_num=cur_seg_prop_num};
    }
    ...
    /*for (i=0; i<calcnum; i++){
        printf("\n%d. %lf %lf %d %ld", i, tasks[i].lend, tasks[i].rend, tasks[i].thr_num, tasks[i].seg_num);
        fflush(stdout);
    }*/
    //printf("%lf %lf %d %ld\n", tasks[1].lend, tasks[1].rend, tasks[1].thr_num, tasks[1].seg_num);
    fprintf(stdout, "\nCompleted\n");
    fflush(stdout);
